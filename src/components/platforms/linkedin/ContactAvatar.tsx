...
/**
 * LinkedIn-specific component to display a contact's avatar with LinkedIn styling
...
const ContactAvatar = ({ contact, size = 40 }: { contact: any, size?: number }) => {
  ...
  // Extract media ID from LinkedIn avatar URLs
  useEffect(() => {
    ...
    // Extract media ID for LinkedIn-specific formats
    let extractedId = null;

    // Handle LinkedIn bridge avatar URLs (similar to Matrix format)
    if (avatarUrl.startsWith('mxc://')) {
      const matches = avatarUrl.match(/mxc:\/\/[^/]+\/([^?]+)/);
      extractedId = matches ? matches[1] : null;
    }
    // Handle LinkedIn thumbnail URLs
    else if (avatarUrl.includes('/thumbnail/')) {
      const matches = avatarUrl.match(/\/thumbnail\/[^/]+\/([^?]+)/);
      extractedId = matches ? matches[1] : null;
    }
    // Handle direct LinkedIn media URLs
    else if (avatarUrl.includes('/media/')) {
      const matches = avatarUrl.match(/\/media\/[^/]+\/([^?]+)/);
      extractedId = matches ? matches[1] : null;
    }
  ...
  // Fetch avatar with IndexedDB caching for LinkedIn
  const fetchAvatarWithCache = async (mediaId: string, contactId: string) => {
    ...
    try {
      ...
      if (cachedAvatar && cachedAvatar.blob) {
        ...
        console.log(`LinkedIn avatar loaded from cache: ${contactId}:${mediaId}`);
        return;
      }

      // Not in cache, fetch from LinkedIn API
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:4000';
      const avatarEndpoint = `${apiUrl}/api/v1/linkedin/media/avatar/${mediaId}?width=${size}&height=${size}`;
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch LinkedIn avatar: ${response.status} ${response.statusText}`);
      }
      ...
      setImageUrl(objectUrl);
      setLoadingState('loaded');
      console.log(`LinkedIn avatar fetched and cached: ${contactId}:${mediaId}`);
    } catch (error) {
      console.error(`Error fetching LinkedIn avatar for ${mediaId}:`, error);
      setLoadingState('error');
    }
  };
  ...
  // If no avatar or error occurred, show LinkedIn-style initials
  if (!mediaId || loadingState === 'error') {
    return (
      ...
      >
        {contact?.display_name?.[0]?.toUpperCase() || 'L'}
      </div>
    );
  }

  // Show loading placeholder while fetching (LinkedIn style)
  if (loadingState === 'loading') {
    return (
      <div
        className="rounded-full bg-blue-100 dark:bg-blue-900 flex items-center justify-center animate-pulse"
        style={containerStyle}
      >
  ...
  return (
    <img
      src={imageUrl || ''}
      alt={contact?.display_name || 'LinkedIn Contact'}
      className="rounded-full object-cover border-2 border-blue-200 dark:border-blue-800"
      style={containerStyle}
